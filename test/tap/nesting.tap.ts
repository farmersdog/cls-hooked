...
test("the example from the docs", function (t) {
  const ns = cls.createNamespace("writer");
  ns.run(function () {
    ns.set("value", 0);
    ...
    function requestHandler() {
      ns.run(function (outer: any) {
        ...
        // deliberately unbound: context must propagate into nextTick on its own
        process.nextTick(function () {
          t.equal(ns.active, outer, "writer.active == outer");
          t.equal(ns.get("value"), 1, "inner has been entered");
          ns.run(function (inner: any) {
